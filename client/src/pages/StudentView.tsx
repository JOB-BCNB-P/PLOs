...
export default function StudentView({ initialEntry = false, studentId }: { initialEntry?: boolean; studentId?: string }) {
  ...
  return <div className="student-view">
    <section className="page-lead student-lead"><div className="lead-copy"><p className="eyebrow">INDIVIDUAL VERDICT · {studentId ? "SUPABASE LIVE" : "DEMO"}</p><h1>คำตัดสินที่<br /><em>เริ่มจากรายบุคคล</em></h1><p>ตรวจสอบผล PLO ของนักศึกษาพร้อมที่มาของคะแนน เครื่องมือ และหลักฐาน โดยยึดกฎที่อนุมัติแล้ว</p></div><div className="student-search-card"><div className="search-caption"><Search size={16} /><span>ค้นหารหัสนักศึกษา</span></div><Input defaultValue={demoStudent.studentCode} /><Button size="icon"><Search size={17} /></Button><small>ผู้ใช้จริงเข้าดูเฉพาะขอบเขตที่ได้รับสิทธิ์</small></div></section>
    ...
    {showEntry && <ScoreEntryForm studentId={studentId} onClose={() => setShowEntry(false)} onSaved={() => setShowEntry(false)} />}
